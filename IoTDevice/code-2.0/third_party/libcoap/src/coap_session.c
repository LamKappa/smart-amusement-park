...
static int32_t loopback_packet_check(char *srcbuf) {
  ...
  if (fd < 0) {
    coap_log(LOG_DEBUG, "socket() fail\n");
    return 1;
  }
  ifc.ifc_len = sizeof(buf);
  ifc.ifc_buf = (char *)buf;
  if (ioctl(fd, SIOCGIFCONF, (char *)&ifc) < 0) {
    coap_log(LOG_DEBUG, "ioctl fail, errno = %d\n", errno);
    goto L_IS_LOOPBACK;
  }
  int interfaceNum = ifc.ifc_len / sizeof(struct ifreq);
  for (int i = 0; i < interfaceNum && i < COAP_INTERFACE_MAX; i++) {
    coap_log(LOG_DEBUG, "interface name: %s\n", buf[i].ifr_name);
    ...
    inet_ntop(AF_INET, tmp_addr_ptr, tmpbuf, INET_ADDRSTRLEN);
    if (memcmp(srcbuf, addr_perface, strlen(addr_perface)) == 0 &&
        strlen(srcbuf) == strlen(tmpbuf) + strlen(addr_perface)) {
      ret = memcmp(srcbuf + strlen(addr_perface), tmpbuf, strlen(srcbuf) - strlen(addr_perface));
    } else {
      if (strlen(srcbuf) == strlen(tmpbuf)) {
        ret = memcmp(srcbuf, tmpbuf, strlen(srcbuf));
      } else {
        ret = 1;
      }
    }
...
static int32_t is_loopback_packet(const coap_packet_t *packet) {
  ...
  coap_address_ntop(&(packet->addr_info.remote), srcbuf, INET6_ADDRSTRLEN);

  return loopback_packet_check(srcbuf);
}
...
coap_endpoint_get_session(coap_endpoint_t *endpoint,
  ...
  if (is_loopback_packet(packet) == 1) {
      coap_log(LOG_DEBUG, "drop loopback packet.");
    return NULL;
  }
