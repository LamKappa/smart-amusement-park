...
static int PakeResponse(AsyBaseCurTask *task, PakeParams *params, const CJson *in, CJson *out, int *status)
{
    ...
    if (((uint32_t)params->baseParams.supportedPakeAlg & PSK_SPEKE) &&
        ...
        if (res != HC_SUCCESS) {
            LOGE("FillPskWithDerivedKey failed, res: %d.", res);
            return res;
        }
    }

    // execute
    res = ServerResponsePakeProtocol(&params->baseParams);
    if (res != HC_SUCCESS) {
        LOGE("ServerResponsePakeProtocol failed, res:%d", res);
        return res;
    }
...
static int PakeServerConfirm(AsyBaseCurTask *task, PakeParams *params, const CJson *in, CJson *out, int *status)
{
    ...
    // execute
    res = ServerConfirmPakeProtocol(&params->baseParams);
    if (res != HC_SUCCESS) {
        LOGE("ServerConfirmPakeProtocol failed, res:%d", res);
        return res;
    }
...
static void DestroyPakeProtocolServerTask(struct AsyBaseCurTaskT *task)
{
    PakeProtocolServerTask *innerTask = (PakeProtocolServerTask *)task;
    if (innerTask == NULL) {
        return;
    }
...
{
    PakeProtocolServerTask *task = (PakeProtocolServerTask *)HcMalloc(sizeof(PakeProtocolServerTask), 0);
    if (task == NULL) {
        return NULL;
    }
    task->taskBase.destroyTask = DestroyPakeProtocolServerTask;
