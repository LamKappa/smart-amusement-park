...
int DevFillQueryDeviceInfo(struct IDevmgrService *inst, struct HdfSBuf *data, struct HdfSBuf *reply)
{
    ...
    if (devMgrSvc == NULL || data == NULL || reply == NULL) {
        HDF_LOGE("%s failed, parameter is null", __func__);
        return HDF_FAILURE;
    }
    ...
    while (HdfSListIteratorHasNext(&itHost)) {
        ...
        while (HdfSListIteratorHasNext(&itDeviceInfo)) {
            ...
            HDF_LOGD("%s %s policy:%d status:%d type:%d",
            ...
            if ((deviceInfo->policy == SERVICE_POLICY_CAPACITY || deviceInfo->policy == SERVICE_POLICY_PUBLIC) &&
                ...
                if (flag == false) {
                    HDF_LOGE("%s writing string failed %s", __func__, deviceInfo->svcName);
                    return HDF_FAILURE;
                }
                flag = HdfSbufWriteInt32(reply, deviceInfo->deviceType);
                if (flag == false) {
                    HDF_LOGE("%s writing int32 failed %d", __func__, deviceInfo->deviceType);
                    return HDF_FAILURE;
                }
            }
        }
