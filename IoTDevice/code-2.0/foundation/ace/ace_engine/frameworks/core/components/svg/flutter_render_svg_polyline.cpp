...
#include "frameworks/core/components/svg/flutter_render_svg_polyline.h"
...
RefPtr<RenderNode> RenderSvgPolyline::Create()
{
    return AceType::MakeRefPtr<FlutterRenderSvgPolyline>();
}

RenderLayer FlutterRenderSvgPolyline::GetRenderLayer()
{
    if (!transformLayer_) {
...
void FlutterRenderSvgPolyline::Paint(RenderContext& context, const Offset& offset)
{
    if (points_.empty()) {
        return;
    }
    ...
    if (pointsVector_.empty()) {
        ...
        SkPath out;
        out.addPoly(&skPoints[0], skPoints.size(), false);
...
bool FlutterRenderSvgPolyline::CreateSkPath(const std::string& pointsStr, std::vector<SkPoint>& skPoints)
{
    if (pointsStr.empty()) {
        return false;
    }
...
bool FlutterRenderSvgPolyline::CreateSkPaths(const std::string& points1,
    const std::string& points2, double weight, SkPath* out)
{
    ...
    begin.addPoly(&skPoints1[0], skPoints1.size(), false);
    end.addPoly(&skPoints2[0], skPoints2.size(), false);
    begin.interpolate(end, weight, out);
    return true;
}

void FlutterRenderSvgPolyline::UpdateMotion(const std::string& path, const std::string& rotate,
...
bool FlutterRenderSvgPolyline::GetStartPoint(Point& point)
{
    if (points_.empty()) {
        return false;
    }
