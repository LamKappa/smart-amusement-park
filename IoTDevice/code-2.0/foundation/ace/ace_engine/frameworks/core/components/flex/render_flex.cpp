...
void RenderFlex::TravelChildrenFlexProps()
{
    for (const auto& child : GetChildren()) {
        ...
        // FLEX_WEIGHT_MODE only active when all the children have set flexWeight
...
void RenderFlex::ResizeByItem(const RefPtr<RenderNode>& item)
{
...
bool RenderFlex::MaybeRelease()
{
    ...
    if (context && context->GetRenderFactory()->GetRenderFlexFactory()->Recycle(this)) {
        ClearRenderObject();
        return false;
    }
    return true;
...
{
    DumpLog::GetInstance().AddDesc(std::string("Direction: ").append(std::to_string(static_cast<int32_t>(direction_))));
    DumpLog::GetInstance().AddDesc(
        std::string("LayoutMode: ").append(std::to_string(static_cast<int32_t>(layoutMode_))));
    DumpLog::GetInstance().AddDesc(
        std::string("MainAxisAlign: ").append(std::to_string(static_cast<int32_t>(mainAxisAlign_))));
    DumpLog::GetInstance().AddDesc(
        std::string("CrossAxisAlign: ").append(std::to_string(static_cast<int32_t>(crossAxisAlign_))));
    DumpLog::GetInstance().AddDesc(
        std::string("MainAxisSize: ").append(std::to_string(static_cast<int32_t>(mainAxisSize_))));
}
