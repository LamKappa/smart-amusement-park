...
void EventManager::TouchTest(const TouchPoint& touchPoint, const RefPtr<RenderNode>& renderNode)
{
    ACE_FUNCTION_TRACE();
    if (!renderNode) {
    ...
    renderNode->TouchTest(point, point, { TouchRestrict::NONE }, hitTestResult);
...
bool EventManager::DispatchKeyEvent(const KeyEvent& event, const RefPtr<FocusNode>& focusNode)
{
    ...
    LOGD("The key code is %{public}d, the key action is %{public}d, the repeat time is %{public}d.",
