...
#include "core/focus/focus_animation_manager.h"
...
void FocusAnimationManager::SetFocusAnimationProperties(const RRect& rrect, const Color& color,
    const Offset& offset) const
{
    ...
    focusAnimation->SetFocusAnimationProperties(rrect, color, offset);
}
...
void FocusAnimationManager::PushFocusAnimationElement(const RefPtr<Element>& element)
{
    ...
    RefPtr<RenderFocusAnimation> cancelAnimation;
    if (focusElement->IsRoot()) {
        if (!rootFocusAnimationStack_.empty()) {
            cancelAnimation = rootFocusAnimationStack_.top().Upgrade();
        }
        rootFocusAnimationStack_.push(renderFocus);
    } else {
        if (!focusAnimationStack_.empty()) {
            cancelAnimation = focusAnimationStack_.top().Upgrade();
        }
        focusAnimationStack_.push(renderFocus);
    }
    if (cancelAnimation) {
        cancelAnimation->CancelFocusAnimation();
    }
}
