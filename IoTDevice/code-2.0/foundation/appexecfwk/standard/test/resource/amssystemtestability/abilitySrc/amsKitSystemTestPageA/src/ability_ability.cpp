...
void AbilityAbility::OnAbilityResult(int requestCode, int resultCode, const Want &resultData)
{
    APP_LOGI("AbilityAbility::OnNewWant");

    BaseAbility::OnAbilityResult(requestCode, resultCode, resultData);
    TestUtils::PublishEvent(APP_ABILITY_RESP_EVENT_NAME, requestCode, "OnAbilityResult");

    DoTestCase();
}

void AbilityAbility::StopSelfAbility()
{
    TerminateAbility();
}
...
void AbilityAbility::TestStopAbility()
{
    ...
    switch ((CaseIndex)std::stoi(AbilityAbility::sequenceNumber_)) {
        ...
        case CaseIndex::TWO: {
            MAP_STR_STR params;
            Want want =
                TestUtils::MakeWant("device", "LifecycleCallbacksAbility", "com.ohos.amsst.page.AppKitA", params);
...
void AbilityAbility::TestStartAbilityWithSetting()
{
    switch ((CaseIndex)std::stoi(AbilityAbility::sequenceNumber_)) {
        case CaseIndex::ONE: {
...
void AbilityAbility::TestStartAbilityForResult()
{
    switch ((CaseIndex)std::stoi(AbilityAbility::sequenceNumber_)) {
        case CaseIndex::ONE: {
        ...
        case CaseIndex::THREE: {
            MAP_STR_STR params;
            Want want = TestUtils::MakeWant("device", "AmsStServiceAbilityA1", "com.ohos.amsst.service.appA", params);
            StartAbilityForResult(want, INT_MIN);
            TestUtils::PublishEvent(APP_ABILITY_RESP_EVENT_NAME, 1, "TestStartAbilityForResult");
            sleep(1);
            StopAbility(want);
        } break;
