...
const int32_t UDID_STRING_LEN = 64;
const uint32_t INITIAL_CBID = 1;

static sem_t g_sem;
static IClientProxy *g_bmsInnerClient = nullptr;
static InstallParam g_installParam = { .installLocation = 1, .keepData = false };
...
const std::string CMD_GET_UDID = "getudid";
...
void CommandParser::HandleCommands(int32_t argc, char *argv[]) const
{
    ...
    } else if (cmd == CMD_GET_UDID) {
        RunAsGetUdidCommand(argc, argv);
#ifdef OHOS_DEBUG
    ...
    while ((option = getopt_long_only(argc, argv, SHORT_OPTIONS.c_str(), LONG_OPTIONS, nullptr)) != -1) {
        switch (option) {
            ...
            case 'p':
                ...
                Install(realPath, &g_installParam, ReceiveCallback);
                sem_wait(&g_sem);
                break;
            default:
    ...
    while ((option = getopt_long_only(argc, argv, SHORT_OPTIONS.c_str(), LONG_OPTIONS, nullptr)) != -1) {
        switch (option) {
            ...
            case 'n':
                ...
                Uninstall(optarg, &g_installParam, ReceiveCallback);
                sem_wait(&g_sem);
                break;
            default:
...
void CommandParser::RunAsGetUdidCommand(int32_t argc, char *argv[]) const
{
    if (argc > MAX_ARGUMENT_NUMBER) {
        printf("%s\n", (ERROR_EXTRA_PARAMETER + DUMP_HELP_MESSAGE).c_str());
        return;
...
void CommandParser::GetInstallBundleInfo(const std::string &bundleName) const
{
    ...
    if (GetBundleInfo(bundleName.c_str(), 1, &bundleInfo) != OHOS_SUCCESS) {
...
void CommandParser::GetInstallBundleInfos(int32_t argc) const
{
    ...
    int32_t len = 0;
    if (GetBundleInfos(1, &bundleInfo, &len) != OHOS_SUCCESS) {
        printf("error message: %s\n", ERROR_DUMP_FAIL.c_str());
        return;
    }
...
void CommandParser::GetBundleInfosByMetaDataKey(const std::string &metaDataKey) const
{
    ...
    if (GetBundleInfosByMetaData(metaDataKey.c_str(), &bundleInfos, &len) != OHOS_SUCCESS) {
        printf("error message: %s\n", ERROR_DUMP_FAIL.c_str());
        return;
    }
